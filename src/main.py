...
def get_out_file_path(img: Path, variant: str) -> Path:
    return out_dir / f'{variant}_{int(target_size_kb)}_{img.stem}.{variant}'

def jpeg_tool(img: Path, quality: int) -> int:
    out_file = get_out_file_path(img=img, variant='jpeg')
...
def j2k_tool(img: Path, quality: int) -> int:
    out_file = get_out_file_path(img=img, variant='j2k')
...
def jxl_tool(img: Path, quality: int) -> int:
    out_file = get_out_file_path(img=img, variant='jxl')
...
def jxr_tool(img: Path, quality: int) -> int:
    out_file = get_out_file_path(img=img, variant='jxr')
    subprocess.run(['JxrEncApp', '-i', str(img), '-o', str(out_file), '-q', str(quality), '-c', '0'], 
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL)
    return out_file.stat().st_size
...
def main(tool: str, target_size: int, file_paths: list[Path]):
    ...
    global out_dir
    global target_size_kb
    target_size_kb = target_size / 1000
